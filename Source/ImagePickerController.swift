...
open class ImagePickerController: UIViewController {
  ...
  open lazy var galleryView: ImageGalleryView = { [unowned self] in
    ...
    galleryView.imageLimit = self.singlePhotoMode ? 1 : self.imageLimit
  ...
  open var singlePhotoMode: Bool = false
  ...
  fileprivate func takePicture() {
    ...
    let action: (Void) -> Void = {
      [unowned self] in self.cameraController.takePicture { self.isTakingPicture = false }
...
extension ImagePickerController: ImageGalleryDelegate {
